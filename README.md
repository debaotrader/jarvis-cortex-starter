...
|-----------|------|---------|
| **Graphify** | Native durable Brain graph, CLI, and MCP | `uv tool install --python 3.12 "graphifyy[mcp,openai]==0.9.11" --with "mcp<2"` |
...
| **mattpocock** | Whatever the `engineering`, `productivity` and `misc` categories hold upstream, minus `caveman` (plugin owns it). Upstream is **pinned** to `$MATTPOCOCK_REF` (default `9603c1c`, 2026-07-16) — it renames skills without notice, and a floating HEAD meant every install could change what lands in a directory the agent reads with no diff here to review. Bumping the pin is deliberate: read the upstream diff, then update `MATTPOCOCK_REF` and `active/contexts/mattpocock-skills.md` together. A SHA is verified after checkout; a tag or branch is accepted and skips that check. The list of skills at the pin is still not restated here — read the clone, cached next to whichever skills dir was targeted — `~/.claude/.cache/mattpocock-skills` for Claude, `~/.codex/.cache/mattpocock-skills` for Codex, or `$MATTPOCOCK_CACHE`: `ls ~/.claude/.cache/mattpocock-skills/skills/{engineering,productivity,misc}`. `active/contexts/mattpocock-skills.md` is current **as of the pin**: it covers all 26 skills at `9603c1c` and carries a table of the names that died upstream (`to-issues`→`to-tickets`, `to-prd`→`to-spec`, `diagnose`→`diagnosing-bugs`, `write-a-skill`→`writing-great-skills`, `zoom-out`→removed). Before the pin that file could not be kept correct at all, since any fix aged out on the next install — so bumping `MATTPOCOCK_REF` and updating that file belong in the same commit | `scripts/install-mattpocock-skills.sh` (best-effort, non-fatal; needs git + network) from `$MATTPOCOCK_REPO` (default `https://github.com/mattpocock/skills.git`, MIT). **Park-always**, like `install-codex-skills.sh`: anything occupying a managed skill name — real dir, real file, foreign symlink, dangling symlink — is moved into `$MATTPOCOCK_BACKUP_DIR` (default `<home>/backups/skills`) with its inode intact and the managed link takes the freed name. It used to skip-and-warn while exiting 0, which left 12 names on unmanaged copies of these very skills, months out of date and outside the pin. Move a parked object back to restore it. |
...
- **`graphify` + `graphify-mcp`** — required for the private durable Brain, from the validated `graphifyy` package with **both** extras: `mcp` serves the graph, `openai` is what `graphify-brain-update.sh` needs to re-extract it. Installing one extra alone drops the other. **`--with "mcp<2"` is a workaround, not a floor:** `graphifyy` 0.9.11 declares `mcp` unpinned, so a plain install resolves to `mcp` 2.0.0 and `graphify-mcp` dies with `ImportError: cannot import name 'AnyUrl' from 'mcp.types'`. Drop the pin once upstream pins it or adopts the 2.x API — and note that `graphify-mcp --help` exits 0 even when the server is dead, so verify with `scripts/doctor.sh`, which speaks the protocol.
...
| `scripts/doctor.sh` | Read-only cross-harness health check (dangling symlinks, settings.json hooks/statusLine, plugin sources, opencode/Cursor wiring). Its `REPOS` section warns when the cortex or the Brain hold commits that were never pushed — the cost of keeping them as separate repos, paid down. **It never fetches:** it compares against the remote-tracking ref already on disk, so it answers "did I forget to push", not "is my branch current". Exit 0 = healthy. |
...
| `scripts/setup-graphify-brain.sh` | Registers the official `graphify-brain` MCP in Claude Code and Codex and enforces the Brain Git-hook policy. Registration only — it never regenerates the graph the MCP serves. |
| `scripts/graphify-brain-update.sh` | Re-extracts the Brain into that graph after new Markdown lands, then re-clusters and names communities. Runs the three steps as one unit because extract alone strips community names. Reads the LLM key from opencode's `auth.json` at runtime (nothing stored in this repo); override with `OPENAI_API_KEY`, and the model with `GRAPHIFY_MODEL`. |
