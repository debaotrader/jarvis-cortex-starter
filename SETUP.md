...
- instala as skills mattpocock via `scripts/install-mattpocock-skills.sh` (best-effort, não-fatal — precisa de git + rede; falha só emite warn e segue). Fixado em `MATTPOCOCK_REF` (default `9603c1c`), não no HEAD do upstream. Colisão de nome é **parqueada** em `<home>/backups/skills` com o inode intacto, nunca pulada nem deletada — mover de volta restaura. Symlinka as categorias ativas (engineering/productivity/misc) menos `caveman` (plugin caveman é o dono); deprecated/in-progress/personal nunca entram. Desligue com `INSTALL_MATTPOCOCK=0`
