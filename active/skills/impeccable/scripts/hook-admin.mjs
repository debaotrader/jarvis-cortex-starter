...
function homeDir() {
  return RUNTIME['HOME'] || os.homedir();
}

function claudeHomeDir() {
  return RUNTIME['CLAUDE_HOME'] || path.join(homeDir(), '.claude');
}

function codexHomeDir() {
  return RUNTIME['CODEX_HOME'] || path.join(homeDir(), '.codex');
}

function cursorHomeDir() {
  return RUNTIME['CURSOR_HOME'] || path.join(homeDir(), '.cursor');
}
...
const HOOK_MANIFEST_TARGETS = [
        ...
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: hookScriptCommand(resolvedSkill, 'hook.mjs', claudeLocalCommand('hook.mjs')),
  ...
  },
  {
    ...
    globalSkillDirs: () => [
      path.join(homeDir(), '.agents', 'skills', 'impeccable'),
      path.join(codexHomeDir(), 'skills', 'impeccable'),
    ],
        ...
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: hookScriptCommand(resolvedSkill, 'hook.mjs', 'node ".agents/skills/impeccable/scripts/hook.mjs"'),
                timeout: TIMEOUT_SECONDS,
                statusMessage: STATUS_MESSAGE,
              },
            ],
  ...
  },
  {
    ...
    globalSkillDirs: () => [
      path.join(cursorHomeDir(), 'skills', 'impeccable'),
    ],
    ...
    manifest: (resolvedSkill) => ({
      ...
      hooks: {
        preToolUse: [
          {
            command: hookScriptCommand(resolvedSkill, 'hook-before-edit.mjs', 'node ".cursor/skills/impeccable/scripts/hook-before-edit.mjs"'),
            timeout: TIMEOUT_SECONDS,
          },
        ],
      },
        ...
        postToolUse: [
          {
            ...
            bash: hookScriptCommand(resolvedSkill, 'hook.mjs', githubLocalCommand('hook.mjs')),
...
function hookScriptCommand(resolvedSkill, scriptName, localCommand) {
  if (!resolvedSkill.isGlobal) return localCommand;
  return nodeCommand(path.join(resolvedSkill.skillDir, 'scripts', scriptName));
}

function claudeLocalCommand(scriptName) {
  const projectDir = ['$', '{CLAUDE_PROJECT_DIR}'].join('');
  return `node "${projectDir}/.claude/skills/impeccable/scripts/${scriptName}"`;
}

function githubLocalCommand(scriptName) {
  const gitRoot = ['$', '(git rev-parse --show-toplevel)'].join('');
  return `node "${gitRoot}/.github/skills/impeccable/scripts/${scriptName}"`;
}
...
function repairHookManifests(cwd) {
  ...
  for (const target of HOOK_MANIFEST_TARGETS) {
    const resolvedSkill = resolveHookSkillDir(cwd, target);
    ...
    const fresh = target.manifest(resolvedSkill);
    if (sharedDest && fileHasImpeccableHookMarker(sharedDest)) {
      if (manifestHasResolvableImpeccableHooks(sharedDest, cwd)) {
        pruneImpeccableHookFromManifest(dest);
        result.already.push(target.provider);
        continue;
      }
      pruneImpeccableHookFromManifest(sharedDest);
    }
...
function resolveHookSkillDir(cwd, target) {
  const local = path.join(cwd, target.skillRel);
  if (fs.existsSync(local)) {
    return { skillDir: local, isGlobal: false };
  }

  for (const candidate of typeof target.globalSkillDirs === 'function' ? target.globalSkillDirs() : []) {
    if (!candidate || !fs.existsSync(candidate)) continue;
    return { skillDir: realPath(candidate), isGlobal: true };
  }
...
function manifestHasResolvableImpeccableHooks(filePath, cwd) {
  let parsed;
  try {
    parsed = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
  } catch {
  ...
  const commands = collectImpeccableHookCommands(parsed);
  if (commands.length === 0) return false;
  return commands.every((command) => {
    const scriptPath = hookCommandScriptPath(command, cwd);
    return scriptPath && fs.existsSync(scriptPath);
  });
}

function collectImpeccableHookCommands(value, out = []) {
  if (Array.isArray(value)) {
    for (const item of value) collectImpeccableHookCommands(item, out);
    return out;
  }
  if (!value || typeof value !== 'object') return out;

  const command = typeof value.command === 'string' ? value.command : '';
  if (command && valueHasImpeccableHookMarker(command)) {
    out.push(command);
  } else if (command && Array.isArray(value.args) && valueHasImpeccableHookMarker(value.args)) {
    const scriptArg = value.args.find((arg) => typeof arg === 'string' && valueHasImpeccableHookMarker(arg));
    if (scriptArg) out.push(`node ${JSON.stringify(scriptArg)}`);
  }

  for (const key of ['bash', 'powershell']) {
    if (typeof value[key] === 'string' && valueHasImpeccableHookMarker(value[key])) {
      out.push(value[key]);
    }
  }

  for (const [key, nested] of Object.entries(value)) {
    if (key === 'command' || key === 'args' || key === 'bash' || key === 'powershell') continue;
    collectImpeccableHookCommands(nested, out);
  }
  return out;
}

function hookCommandScriptPath(command, cwd) {
  const rawPath = firstCommandPath(command);
  if (!rawPath) return null;
  return expandHookCommandPath(rawPath, cwd);
}

function firstCommandPath(command) {
  const tokens = String(command || '').match(/"[^"]*"|'[^']*'|\S+/g) || [];
  if (tokens.length === 0) return null;
  const binary = stripCommandQuotes(tokens[0]);
  if (!/(^|\/)(node|bash|sh|python3?)$/.test(binary)) {
    return valueHasImpeccableHookMarker(binary) ? binary : null;
  }

  for (let i = 1; i < tokens.length; i++) {
    const arg = stripCommandQuotes(tokens[i]);
    if (!arg || arg.startsWith('-')) continue;
    if (/(^|\/)(bash|sh)$/.test(binary) && valueHasImpeccableHookMarker(arg)) {
      return firstCommandPath(arg) || arg;
    }
    return arg;
  }
  return null;
...
function expandHookCommandPath(rawPath, cwd) {
  let expanded = String(rawPath || '')
    .split('${CLAUDE_PROJECT_DIR}').join(cwd)
    .split('$CLAUDE_PROJECT_DIR').join(cwd)
    .split('$(git rev-parse --show-toplevel)').join(cwd)
    .split('${HOME}').join(homeDir())
    .split('$HOME').join(homeDir())
    .split('${CLAUDE_HOME}').join(claudeHomeDir())
    .split('$CLAUDE_HOME').join(claudeHomeDir())
    .split('${CODEX_HOME}').join(codexHomeDir())
    .split('$CODEX_HOME').join(codexHomeDir());
