 ...
 *
 * Honors `JARVIS_CORTEX_ROOT` env var so hooks run in CI (where no
 * `~/.claude/` symlink exists) can resolve paths from the repo checkout.
 * Falls back to `~/.claude` for legacy user installs.
 *
 * SECURITY: this function trusts the value of `JARVIS_CORTEX_ROOT`
 * verbatim. Do not call from contexts where the env var is attacker-
 * controllable. The current callers (enforce.js, session-start.js,
 * pre-compact.js, jarvis-learn/scan.js) all run with the user's own
 * env, so this is acceptable.
 */
function getClaudeDir() {
  if (process.env.JARVIS_CORTEX_ROOT) {
    return process.env.JARVIS_CORTEX_ROOT;
  }
...
/**
 * Load local config from config.json (paths, preferences)
 * Falls back to defaults if config.json doesn't exist
 * @returns {object} Config object with claudeHome, graphifyBrainPath
 */
function loadConfig() {
  const configPath = path.join(getClaudeDir(), 'config.json');
  if (fs.existsSync(configPath)) {
    try {
      return JSON.parse(fs.readFileSync(configPath, 'utf-8'));
    } catch {
      return { claudeHome: getClaudeDir(), graphifyBrainPath: '' };
    }
  }
  return {
    claudeHome: getClaudeDir(),
    graphifyBrainPath: ''
  };
}
